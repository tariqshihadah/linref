...
* Added `DataFrame.lr.geometry_scale` and `DataFrame.lr.invalid_geometry_scale` properties for spatial linear events. `geometry_scale` reports each event's measure length divided by its geometry length (a valid event has a finite, positive scale), and `invalid_geometry_scale` flags events whose scale is zero (collapsed measure), infinite (zero-length geometry), or undefined. Both return `None` when the LRS is not both linear and spatial. Added the accompanying `GeometryScaleError` and `GeometryScaleWarning` types.
